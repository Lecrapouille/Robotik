...
std::string STLLoader::s_last_error;

// ----------------------------------------------------------------------------
bool STLLoader::loadSTL(const std::string& p_filename, MeshData& p_mesh_data)
{
    p_mesh_data.clear();
    s_last_error.clear();
    ...
    {
        return loadBinarySTL(file, p_mesh_data);
    }
    else
    {
        ...
        if (!file.is_open())
        {
            s_last_error = "Cannot reopen file in text mode: " + p_filename;
            return false;
        }
        return loadAsciiSTL(file, p_mesh_data);
    }
}
...
// ----------------------------------------------------------------------------
bool STLLoader::loadBinarySTL(std::ifstream& p_file, MeshData& p_mesh_data)
{
    // Skip 80-byte header
    p_file.seekg(80);
    ...
    if (triangle_count == 0)
    {
        s_last_error = "STL file contains no triangles";
        return false;
    }

    p_mesh_data.triangle_count = triangle_count;
    p_mesh_data.vertices.reserve(triangle_count *
                                 18); // 3 vertices * 6 floats per vertex
    p_mesh_data.indices.reserve(triangle_count * 3);
    ...
    for (uint32_t i = 0; i < triangle_count; ++i)
    {
        ...
        {
            // Position
            p_mesh_data.vertices.push_back(vertices[j * 3 + 0]);
            p_mesh_data.vertices.push_back(vertices[j * 3 + 1]);
            p_mesh_data.vertices.push_back(vertices[j * 3 + 2]);

            // Normal
            p_mesh_data.vertices.push_back(normal[0]);
            p_mesh_data.vertices.push_back(normal[1]);
            p_mesh_data.vertices.push_back(normal[2]);

            // Add index
            p_mesh_data.indices.push_back(static_cast<unsigned int>(i * 3 + j));
        }
    }
...
// ----------------------------------------------------------------------------
bool STLLoader::loadAsciiSTL(std::ifstream& p_file, MeshData& p_mesh_data)
{
    std::string line;
    float normal[3];
    float vertex[3];
    ...
    while (std::getline(p_file, line))
    {
        ...
        else if (token == "vertex")
        {
            ...
            p_mesh_data.vertices.push_back(vertex[0]);
            p_mesh_data.vertices.push_back(vertex[1]);
            p_mesh_data.vertices.push_back(vertex[2]);
            p_mesh_data.vertices.push_back(normal[0]);
            p_mesh_data.vertices.push_back(normal[1]);
            p_mesh_data.vertices.push_back(normal[2]);

            // Add index
            p_mesh_data.indices.push_back(
                static_cast<unsigned int>(p_mesh_data.indices.size()));
        }
