...
public:
    ...
    // ------------------------------------------------------------------------
    std::unique_ptr<robotik::Robot>
    load(const std::string& p_filename) override;
    ...
    //! Robot (name and hierarchy).
    // ------------------------------------------------------------------------
    template <typename RobotType = robotik::Robot>
    std::unique_ptr<RobotType> loadAs(const std::string& p_filename)
    {
        static_assert(std::is_base_of<robotik::Robot, RobotType>::value,
                      "RobotType must derive from robotik::Robot");
...
private:
    ...
    std::unique_ptr<robotik::Robot>
    buildRobotModel(std::string const& p_robot_name);
    std::unique_ptr<robotik::Joint>
    buildJointTree(robotik::Joint const* p_current_joint);

    // Link and geometry parsing
    bool parseLinkGeometries(pugi::xml_node p_link_element,
                             URDFLoaderLink& p_link) const;
    std::unique_ptr<robotik::Geometry>
    parseGeometry(pugi::xml_node p_geometry_element,
...
private:
    ...
    std::unordered_map<std::string, std::unique_ptr<robotik::Joint>> m_joints;
    std::string m_error;
};
