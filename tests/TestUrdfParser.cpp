...
protected:
    ...
    void SetUp() override
    {
        ...
        urdf_file_path = "data/scara_robot.urdf";
    }
...
TEST_F(URDFParserTest, ParseSimpleRobot)
{
    ...
    EXPECT_EQ(joint1->getType(), Joint::Type::REVOLUTE);
    EXPECT_EQ(joint1->getAxis(), Eigen::Vector3d(0, 0, 1));
}
...
TEST_F(URDFParserTest, ParseSCARArobot)
{
    ...
    auto joint_names = robot->getJointNames();
    EXPECT_EQ(joint_names.size(), 4); // 4 movable joints + 1 fixed

    // Check specific joints exist
    EXPECT_NE(robot->getJoint("joint1"), nullptr);
    EXPECT_NE(robot->getJoint("joint2"), nullptr);
    EXPECT_NE(robot->getJoint("joint3"), nullptr);
    EXPECT_NE(robot->getJoint("joint4"), nullptr);
    EXPECT_EQ(robot->getJoint("joint5"), nullptr);

    // Check joint types
    auto joint1 = robot->getJoint("joint1");
    auto joint2 = robot->getJoint("joint2");
    auto joint3 = robot->getJoint("joint3");
    auto joint4 = robot->getJoint("joint4");
    auto joint5 = robot->getJoint("joint5");

    EXPECT_EQ(joint1->getType(), Joint::Type::REVOLUTE);
    EXPECT_EQ(joint2->getType(), Joint::Type::REVOLUTE);
    EXPECT_EQ(joint3->getType(), Joint::Type::REVOLUTE);
    EXPECT_EQ(joint4->getType(), Joint::Type::PRISMATIC);
    // EXPECT_EQ(joint5->getType(), Joint::Type::FIXED);
}
...
TEST_F(URDFParserTest, JointLimits)
{
    ...
    EXPECT_NO_THROW(joint1->setValue(3.0));
    EXPECT_NO_THROW(joint1->setValue(-3.0));
    ...
    EXPECT_NO_THROW(joint4->setValue(0.05));
    EXPECT_NO_THROW(joint4->setValue(-0.05));
}
...
TEST_F(URDFParserTest, CompleteSCARAFunctionality)
{
    ...
    std::vector<double> scara_config = { 0.0, M_PI / 4, -M_PI / 4, 0.02 };
    ...
    Pose end_effector_pose = robot->getEndEffectorPose();
    ...
    std::vector<double> retrieved_values = robot->getJointValues();
